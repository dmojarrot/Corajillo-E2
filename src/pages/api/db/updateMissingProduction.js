...
export default async function handler(req, res) {
  try {
    ...
    const { rows } = await missingProductionDB.query(
      `UPDATE missingProduction SET "NOMBRE" = '${nombre}, "PIEZAS" = '${piezas}, "FECHA_DE_ENTREGA" = '${fechaEntrega}', "NUMERO_DE_LINEA" = '${numeroLinea}, "TIEMPO_HORAS" = '${tiempo}' WHERE id = ${id} RETURNING *`
    )
    console.log(rows)
